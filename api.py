...
def temizle_veri(data):
    """Developer bilgilerini temizle, sadece veriyi döndür"""
    if isinstance(data, dict):
        # Developer bilgilerini kaldır
        data.pop('developer', None)
        data.pop('version', None)
        
        # İç içe dict'leri temizle
        for key, value in data.items():
            if isinstance(value, dict):
                data[key] = temizle_veri(value)
            elif isinstance(value, list):
                data[key] = [temizle_veri(item) if isinstance(item, dict) else item for item in value]
        return data
    elif isinstance(data, list):
        return [temizle_veri(item) if isinstance(item, dict) else item for item in data]
    return data
...
def tc_bilgi():
    ...
    sonuc = punisher_request('tc.php', {'tc': tc})
    temiz = temizle_veri(sonuc)
    
    # Sadece veriyi döndür
    if isinstance(temiz, dict) and 'results' in temiz:
        return jsonify(temiz['results'])
    return jsonify(temiz)
...
def tc_pro():
    ...
    sonuc = punisher_request('tcpro.php', {'tc': tc})
    temiz = temizle_veri(sonuc)
...
def tc_tum():
    ...
    for name, endpoint in endpoints.items():
        veri = punisher_request(endpoint)
        temiz = temizle_veri(veri)
        
        # results içindeki veriyi düzleştir
        if isinstance(temiz, dict) and 'results' in temiz:
            sonuc[name] = temiz['results']
        else:
            sonuc[name] = temiz
...
def adsoyad_bilgi():
    ...
    sonuc = punisher_request('adsoyad.php', {'ad': ad, 'soyad': soyad})
    temiz = temizle_veri(sonuc)
...
def adsoyad_pro():
    ...
    sonuc = punisher_request('adsoyadpro.php', {'ad': ad, 'soyad': soyad})
    temiz = temizle_veri(sonuc)
...
def aile_bilgi():
    ...
    sonuc = punisher_request('aile.php', {'tc': tc})
    temiz = temizle_veri(sonuc)
...
def aile_pro():
    ...
    sonuc = punisher_request('ailepro.php', {'tc': tc})
    temiz = temizle_veri(sonuc)
...
def anne_sorgu():
    ...
    if isinstance(tc_bilgi, dict):
        # Doğrudan kontrol
        anne_tc = tc_bilgi.get('ANNETC')
        if not anne_tc and 'results' in tc_bilgi and isinstance(tc_bilgi['results'], list):
            for item in tc_bilgi['results']:
                if isinstance(item, dict):
                    anne_tc = item.get('ANNETC')
                    break
    
    if anne_tc:
        sonuc = punisher_request('tc.php', {'tc': anne_tc})
        temiz = temizle_veri(sonuc)
        if isinstance(temiz, dict) and 'results' in temiz:
            return jsonify({
                'anne_tc': anne_tc,
                'bilgiler': temiz['results']
            })
        return jsonify({'anne_tc': anne_tc, 'bilgiler': temiz})
...
def baba_sorgu():
    ...
    if isinstance(tc_bilgi, dict):
        baba_tc = tc_bilgi.get('BABATC')
        if not baba_tc and 'results' in tc_bilgi and isinstance(tc_bilgi['results'], list):
            for item in tc_bilgi['results']:
                if isinstance(item, dict):
                    baba_tc = item.get('BABATC')
                    break
    
    if baba_tc:
        sonuc = punisher_request('tc.php', {'tc': baba_tc})
        temiz = temizle_veri(sonuc)
        if isinstance(temiz, dict) and 'results' in temiz:
            return jsonify({
                'baba_tc': baba_tc,
        ...
        return jsonify({'baba_tc': baba_tc, 'bilgiler': temiz})
...
def cocuk_sorgu():
    ...
    sonuc = punisher_request('cocuk.php', {'tc': tc})
    temiz = temizle_veri(sonuc)
...
def es_sorgu():
    ...
    sonuc = punisher_request('es.php', {'tc': tc})
    temiz = temizle_veri(sonuc)
...
def kardes_sorgu():
    ...
    sonuc = punisher_request('kardes.php', {'tc': tc})
    temiz = temizle_veri(sonuc)
...
def sulale_bilgi():
    ...
    sonuc = punisher_request('sulale.php', {'tc': tc})
    temiz = temizle_veri(sonuc)
...
def sulale_pro():
    ...
    sonuc = punisher_request('sulalepro.php', {'tc': tc})
    temiz = temizle_veri(sonuc)
...
def adres_sorgu():
    ...
    sonuc = punisher_request('adres.php', {'tc': tc})
    temiz = temizle_veri(sonuc)
...
def isyeri_sorgu():
    ...
    sonuc = punisher_request('isyeri.php', {'tc': tc})
    temiz = temizle_veri(sonuc)
...
def tapu_sorgu():
    ...
    sonuc = punisher_request('tapu.php', {'tc': tc})
    temiz = temizle_veri(sonuc)
...
def tc_gsm():
    ...
    sonuc = punisher_request('tcgsm.php', {'tc': tc})
    temiz = temizle_veri(sonuc)
...
def gsm_tc():
    ...
    sonuc = punisher_request('gsmtc.php', {'gsm': gsm})
    temiz = temizle_veri(sonuc)
...
def dogum_ilce():
    ...
    sonuc = punisher_request('dogumtililce.php', {'dogumt': dogumt, 'il': il, 'ilce': ilce})
    temiz = temizle_veri(sonuc)
...
def soyad_dogum():
    ...
    sonuc = punisher_request('soyaddogumt.php', {'dogumt': dogumt, 'soyad': soyad})
    temiz = temizle_veri(sonuc)
...
def iban_sorgu():
    ...
    sonuc = punisher_request('iban.php', {'iban': iban})
    temiz = temizle_veri(sonuc)
...
def operator_sorgu():
    ...
    sonuc = punisher_request('gncloperator.php', {'numara': numara})
    temiz = temizle_veri(sonuc)
